...
FineNotes's **Custom endpoint** vendor points Cloud AI recognition at any
...
4. In FineNotes settings: **Handwriting recognition → Cloud AI**, vendor
...
2. In FineNotes settings: **Handwriting recognition → Cloud AI**, vendor
   **Custom endpoint (OpenAI-compatible)**, endpoint URL
...
3. **Set the model name exactly as the service lists it** — for Hetzner today
   ...
   one trap: leaving the field on FineNotes's Ollama-shaped default
...
- Thinking tokens count against the response budget. FineNotes caps output at
  8192 tokens per page; if a model spends all of it reasoning, recognition
  fails with an explicit "hit FineNotes's output limit" message rather than
...
4. Use the HTTPS URL Tailscale prints (e.g.
   `https://yourbox.your-tailnet.ts.net/v1`) as the endpoint URL in FineNotes.
...
- The `tailscale serve` mapping can stay configured while the server is
  down — clients just get a 502 until you start it again, and FineNotes
...
- **"hit FineNotes's output limit"** — the model ran out of response budget
