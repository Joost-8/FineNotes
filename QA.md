# GoodObsidian — on-device QA checklist
...
- [ ] Enable the HUD (command: **"Show or hide the input debug overlay"**) for the capture
...
## P0 — After the InkedMark rewrite (before the public release)

Every InkedMark-era module was rewritten on 2026-09-25 against
characterisation tests (see `INKEDMARK_REWRITE.md`). The tests cannot see an
iPad. Run this section with the HUD on, and record the screen.

**Pencil input** (`pointer-controller.ts`, `palm-rejection.ts`)

- [ ] Write fast with "T", "t", "i" dots, "x" and "f": every stroke is there.
      A regression looks like a missing crossbar, or two strokes joined by a
      line.
- [ ] Fast cursive: HUD move records show `c` above 1 (coalesced samples
      arrive). If `c` is always 1, fast writing turns jagged.
- [ ] Draw and hold still (hold-to-snap): the HUD shows a `cancel` and the
      shape still snaps.
- [ ] A palm on the glass before the pen lands: no jump; ink starts under the
      pen. A palm landing mid-stroke: no scroll or zoom, then or after the
      lift.
- [ ] Pinch while the pen rests: nothing. Pinch, lift one finger: it carries
      on as a pan without a jump. A third finger: nothing.
- [ ] Apple Pencil hover (M2 or later iPad Pro): no ink, no scroll.

**Drawing surface** (`ink-surface.ts`)

- [ ] First open from cold, in a new pane and in split view: the page is
      drawn, not blank.
- [ ] The eraser, standard and whole-stroke, with both filters; then undo.
...
**Rendering** (`renderer.ts`, `freehand.ts`, `stroke-builder.ts`)

- [ ] Committed ink stays after the pen lifts, with no torn fragments.
- [ ] A lasso, a dragged selection and a dragged picture leave nothing
      behind after release.
- [ ] Highlighter ink still tints the writing under it.

**Opening notes** (`main.ts`, `ink-view.ts`, the note format)

- [ ] Open a notebook from the file list, from a link, and with
      Back/Forward. It lands on the right page, with no Back loop.
- [ ] "Switch between notebook and Markdown view": go to Markdown, change the
      layout, come back.
- [ ] An existing notebook opens and looks exactly as before. Edit it and it
      saves; it reopens identical on the desktop.
- [ ] Protected load, after a partial iCloud/Syncthing sync: - the notice appears once; - edits, new pages and transcription are refused; - the file on disk is unchanged; - reopening after the sync unlocks it.
- [ ] What's new shows once after the update, and the Scribble notice once,
      on the first open.

**Settings and chrome** (`settings.ts`, `styles.css`, `toolbar.ts`)

- [ ] The settings tab from top to bottom, including: - the Scribble callout; - a paper width of 5000, which shows its callout; - switching the AI service to Custom endpoint with an `http://` URL,
      which shows the plain-HTTP warning; - Add, Replace and Remove key; - Connect OpenRouter.
- [ ] The toolbar and its options look exactly as in 0.9.0.
- [ ] The HUD overlay looks as before: it moved to `inset` and the `font`
      shorthand.

**AI** (`src/recognition/`). One real transcription per vendor you use.
Before each one, put three blank lines in the note's own text; they must
survive, and the transcript lands in its block.
...
- [ ] Open the `.notebook.md` as **markdown** (Switch between notebook and Markdown view) — the
      `%%goodobsidian … %%` block is present and the body prose is untouched.
...
- [ ] `![[Note.notebook.md]]` in a host note creates an embed edge in the graph.
...
- [ ] Output limit: with a reasoning model on a dense page, a cut-off response
      shows the "hit GoodObsidian's output limit" notice and leaves the managed
...
- [ ] Connect OpenRouter: browser opens, approving stores a key and sets the
      ...
      `obsidian://goodobsidian-openrouter?code=x` with no pending connect shows a
      clean notice.
...
- [ ] Desktop only: settings search (top of the Obsidian settings window)
      finds GoodObsidian settings by name (e.g. "highlighter opacity"). Mobile
...
- [ ] A `data.json` saved with `"recognitionProviderId": "trocr-local"`
      (on-device recognition, removed 2026-09) opens with Manual selected, and
      the next save drops its `experimentalTrocr` and `trocrModel` keys.
- [ ] Input debug overlay toggle updates open ink views immediately.

## P2 — What's new modal

- [ ] With existing plugin data, lower `lastSeenVersion` in
      `.obsidian/plugins/finenotes/data.json` (e.g. to `1.0.0`), reload
...
- [ ] **Show the changelog** command shows the full released changelog; markdown
...
- [ ] Ribbon icon and **"Create handwriting note"** make a new `.notebook.md` (a single page: `.page.md`) and open
      it in the ink view; opening an existing `.notebook.md`, `.page.md` or
      pre-0.6.3 `.ink.md` switches to the ink view.
